...
const Navbar = () => {
    ...
    const { setShowSearch, cartItem, clientId } = useContext(ShopContext);

    const logout = () => {
        localStorage.removeItem('clientId'); //Delete localStorage state
    };
                ...
                <div className='group relative'>
                    ...
                    <div className='group-hover:block hidden absolute dropdown-menu right-0 pt-4'>
                        <div className='flex flex-col gap-2 w-36 py-3 px-5  bg-slate-100 text-gray-500 rounded'>
                            <p onClick={() => navigate('/profile')} className='cursor-pointer hover:text-black'>profil</p>
                            <p onClick={() => navigate('/bill')} className='cursor-pointer hover:text-black'>commande</p>
                            <p onClick={handleAuth} className='cursor-pointer hover:text-black'>
                                {clientId ? 'Déconnexion' : 'Connexion'}
                            </p>
                        </div>
                    </div>
                </div>
